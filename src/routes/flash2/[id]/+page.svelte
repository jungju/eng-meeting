<script lang="ts">
  ...
  /* ─── 재생 로직 ─── */
  function randIdx() {
    if ((limit >= 0 && playedSet.size >= limit) || playedSet.size === s.length)
      return -1;
    let i:number;
  ...
  function toggleViewMode(backward=false) {
    stop();
    viewMode = (viewMode + (backward ? -1 : 1) + 3) % 3;
  }
...
<main class="wrapper">
  {#if !finished}
    ...
    >
      ...
      {#if idx !== -1}
  {#if showKor}
    <p class="kor">{k[idx]}</p>               <!-- 한국어 문장 -->
  {/if}

  {#if showEng}
    <p class="eng">{s[idx]}</p>               <!-- 영어 문장 -->
  {:else if hintLevel > 0}
    <p class="eng">{maskedSentence(s[idx])}</p>   <!-- 힌트용 -->
  {:else}
