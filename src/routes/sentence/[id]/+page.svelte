...
const gaps=[0,1e3,3e3,5e3];let gap=0,display:"both"|"hideKor"|"hideEng"="both",audio:"eng"|"kor"|"both"="eng",show=true,showExp=true,MAX_ALL=100,opts=[1,3,5,10];let opt=0,rep=opts[0],repCnt=0,allCnt=0,q:string[]=[];
$:rep=opts[opt];$:listBtm=show?`calc(60px + env(safe-area-inset-bottom))`:`env(safe-area-inset-bottom)`;$:if(r!=="all")allCnt=0;
onMount(async()=>{if(!id)return;const res=await fetch(`${BASE}/sentences.json`);if(res.ok){const d=await res.json();s=d.sentences;k=d.korean;e=d.explanations??[]}});onDestroy(()=>{player?.pause();gapT&&clearTimeout(gapT)});
...
const tRepeat=()=>{r=r==="none"?"one":r==="one"?"all":"none";repCnt=0};
...
const tExp=()=>showExp=!showExp;
$:gapLabel=gap?gaps[gap]/1e3+"s":"즉시";$:dispLbl=display==="both"?"한/영":display==="hideKor"?"영":"한";$:audLbl=audio==="eng"?"영":audio==="kor"?"한":"모두";$:expLbl=showExp?"설명ON":"설명OFF";
$:buttons=[{id:"play",icon:p?"⏸":"▶"},{id:"repeat",text:"반복:"+(r==="none"?"없음":r==="one"?"문장":"전체")},{id:"count",text:"횟수:"+rep+"x",active:true},{id:"disp",text:dispLbl},{id:"blank",text:b?"원문":"빈칸"},{id:"audio",text:"음성:"+audLbl},{id:"gap",text:"간격:"+gapLabel},{id:"exp",text:expLbl}];
...
<div class="list" style={`position:absolute;top:50px;left:0;right:0;bottom:${listBtm};overflow-y:auto`}>{#if s.length}{#each s as t,i}<div id={`s-${i}`} role="button" tabindex="0" on:click={()=>play(i)} on:keydown={(e)=>["Enter"," "].includes(e.key)&&play(i)} style={`position:relative;padding:1rem;margin:.5rem 0;border-radius:.5rem;cursor:pointer;background:${p&&i===idx?(repCnt+1===rep?"#ffe3e3":"#d0ebff"):"#f9f9f9"};${p&&i===idx?"font-weight:bold;":""}`}><div style="display:flex;align-items:center"><span style="margin-right:.5rem;color:#6b7280;font-weight:bold">{i+1}.</span><div style="flex:1 1 auto"><div style={`font-size:3.2rem;display:${display==="hideEng"?"none":"block"}`}>{t}</div><div style={`font-size:3.2rem;color:#374151;margin-top:.25rem;display:${display==="hideKor"?"none":"block"}`}>{k[i]}</div>{#if e[i]}<div style={`font-size:2.6rem;color:#6b7280;margin-top:.25rem;display:${showExp?"block":"none"}`}>{e[i]}</div>{/if}</div></div>{#if (p&&i===idx)}<span style="position:absolute;right:8px;top:8px;font-size:1.2rem;opacity:.7">{repCnt+1}</span>{/if}</div>{/each}{:else}<p style="padding:1rem">로딩...</p>{/if}</div>
