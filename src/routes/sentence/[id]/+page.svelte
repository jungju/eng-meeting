<script lang="ts">
  ...
  let b = false,
    o: string[] = [],
    gapT: ReturnType<typeof setTimeout> | null = null;
  const gaps = [0, 1e3, 3e3, 5e3];
  let gap = 0;
  let display: "both" | "hideKor" | "hideEng" = "both",
    audio: "eng" | "kor" | "both" = "eng",
    show = true,
    showExp = true; // explanation visibility
  const MAX_ALL = 100,
    opts = [1, 2, 3, 5];
  let opt = 0,
    rep = opts[0];
  ...
  const tPlay = () =>
    p
      ? (player.pause(), (p = false), (q = []), gapT && clearTimeout(gapT))
      : play(idx === -1 ? 0 : idx);
  const tRepeat = () =>
    (r = r === "none" ? "one" : r === "one" ? "all" : "none");
  const tOpt = () => {
    if (r !== "one") opt = (opt + 1) % opts.length;
  };
  const tAudio = () =>
    (audio = audio === "eng" ? "kor" : audio === "kor" ? "both" : "eng");
  const tBlank = () => {
    b = !b;
    if (b) {
      o = [...s];
  ...
  /* ──── ControlBar 동적 버튼 ──── */
  $: gapLabel = gap ? gaps[gap] / 1000 + "s" : "즉시";
  $: dispLbl =
    display === "both" ? "한/영" : display === "hideKor" ? "영" : "한";
  ...
  $: buttons = [
    ...
    { id: "exp", text: expLbl } // explanation toggle
  ];
      ...
      >
        <div class="ln">
          ...
          <div>
            ...
            {#if e[i]}
              <div class="ex" class:hidden={!showExp}>{e[i]}</div>
            {/if}
          </div>
        </div>
      </div>
